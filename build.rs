...
//! implementation file, so we compile only that file. If someone later adds `src/*.c` files
//! to this build as well, every function will be defined twice and the link will fail.
//!
...
//!
//! We currently build only the portable C implementation. mldsa-native automatically
//! selects that implementation whenever no architecture-specific backend is enabled, so the
//! library still works correctly on every supported target. Faster verified AVX2/NEON
//! implementations can be enabled later without changing the Rust API.
//!
